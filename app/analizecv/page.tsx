...
import React, { useState } from 'react';
import { Upload, CheckCircle, AlertCircle, BookOpen, TrendingUp, Award, Loader, ArrowRight, Check } from 'lucide-react';
...
export default function CVAnalysisDashboard() {
  ...
  const handleGenerateQuiz = () => {
    if (selectedSkills.size === 0) {
      setError('Please select at least one skill to generate a quiz');
      return;
    }

    // Convert selected skills to comma-separated string and pass to quiz page
    const skillsList = Array.from(selectedSkills).join(', ');
    window.location.href = `/project?topic=${encodeURIComponent(skillsList)}`;
  };
                ...
                {response.skillGapReport.missingSkills && response.skillGapReport.missingSkills.length > 0 && (
                  <div>
                      ...
                      {response.skillGapReport.missingSkills.map((skill, i) => (
                        <div key={i} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          <div className="flex justify-between items-start gap-4">
                            ...
                            {skill.isRequired && (
                              <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded shrink-0">Required</span>
                            )}
                          </div>
                          {skill.recommendedResources && skill.recommendedResources.length > 0 && (
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        <Check size={20} />
                        Generate Quiz ({selectedSkills.size} selected)
                      </button>
